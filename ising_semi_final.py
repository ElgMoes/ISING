...
def try_flip(arr, i, j, T, sim, loop): # attempts to flip an object and flips it if possible # metropolis
    ...
    if (dE < 0 or R < p): # flip is benificial for energy or the random chance flips the object
        global energy
        energy += dE
        energy_array[0][sim][loop] = energy
...
def metropolis(sim=1, loop=0):
    ...
    spin_lattice = try_flip(spin_lattice, i, j, T[sim%len(T)], sim, loop)
    ...
    mean_magnetisation_array[0][sim][loop] = mean_magnetisation
...
def flip_wolff_cluster(lattice, i, j, T, sim=1, loop=1):
    ...
    if dE <= 0 or R < p:
        global energy
        energy += dE
        energy_array[1][sim][loop] = energy
...
def wolff_step(lattice, T, sim=1, loop=1):
    ...
    mean_magnetisation_array[1][sim][loop] = np.absolute(np.mean(lattice))
    return lattice

# setting up arrays to store desired variables
mean_energy_per_sim = np.zeros(shape = (2, simulations))
mean_energy_per_sim_last = np.zeros(shape = (2, simulations))
mean_energy_per_sim_last_std = np.zeros(shape = (2, simulations))
mean_abs_mean_magnetisation = np.zeros(shape = (2, simulations))
mean_abs_mean_magnetisation_std = np.zeros(shape = (2, simulations))
heat_capacitance = np.zeros(shape = (2, simulations))
heat_capacitance_std = np.zeros(shape = (2, simulations)) 

energy_array = np.zeros(shape=(2, simulations, loops))
mean_magnetisation_array = np.zeros(shape=(2, simulations, loops))
...
for seed in range(1):
    ...
    for sim in range(0, simulations): # metropolis algorithm
        ...
        energy_array[0][sim][0] = energy
        mean_magnetisation_array[0][sim][0] = np.mean(spin_lattice)

        for loop in tqdm(range(loops), desc=f"Processing Steps Metropolis {sim+1}/{simulations}"): # looping the flips
            spin_lattice = metropolis(sim=sim, loop=loop)

        # getting averages of desired variables
        mean_energy_per_sim[0][sim] = np.mean(energy_array[0][sim])
        mean_energy_per_sim_last[0][sim] = np.mean(energy_array[0][sim][-last:])
        mean_energy_per_sim_last_std[0][sim] = np.std(energy_array[0][sim][-last:])
        mean_abs_mean_magnetisation[0][sim] = np.mean(np.abs(mean_magnetisation_array[0][sim][-last:]))
        mean_abs_mean_magnetisation_std[0][sim] = np.std(np.abs(mean_magnetisation_array[0][sim][-last:]))
        heat_capacitance[0][sim] = (np.var(energy_array[0][sim][-last:]))/T[sim]
        heat_capacitance_std[0][sim] = (mean_energy_per_sim_last_std[0][sim])/T[sim]
    ...
    for sim in range(0, simulations): # wolff algorithm
        ...
        # setting up energy, magnetisation, and time arrays
        energy = calculate_system_energy(spin_lattice)
        energy_array[1][sim][0] = energy
        mean_magnetisation_array[1][sim][0] = np.mean(spin_lattice)
        ...
        # getting averages of desired variables
        mean_energy_per_sim[1][sim] = np.mean(energy_array[1][sim])
        mean_energy_per_sim_last[1][sim] = np.mean(energy_array[1][sim][-last:])
        mean_energy_per_sim_last_std[1][sim] = np.std(energy_array[1][sim][-last:])
        mean_abs_mean_magnetisation[1][sim] = np.mean(np.abs(mean_magnetisation_array[1][sim][-last:]))
        mean_abs_mean_magnetisation_std[1][sim] = np.std(np.abs(mean_magnetisation_array[1][sim][-last:]))
        heat_capacitance[1][sim] = (np.var(energy_array[1][sim]))/T[sim]
        heat_capacitance_std[1][sim] = (mean_energy_per_sim_last_std[1][sim])/T[sim]
...
for sim in range(simulations):
    ax1.plot(time_array, energy_array[0][sim], linewidth=0.5)
    ax2.plot(time_array, mean_magnetisation_array[0][sim], linewidth=0.5)
    ax3.plot(time_array, energy_array[1][sim], linewidth=0.5)
    ax4.plot(time_array, mean_magnetisation_array[1][sim], linewidth=0.5)
ax1.set_xlabel("Time")
ax2.set_xlabel("Time")
ax3.set_xlabel("Time")
ax4.set_xlabel("Time")
...
ax5.scatter(T, mean_energy_per_sim_last[0])
ax5.errorbar(T, mean_energy_per_sim_last[0], yerr=mean_energy_per_sim_last_std[0], fmt='o')
ax5.scatter(T, mean_energy_per_sim_last[1])
ax5.errorbar(T, mean_energy_per_sim_last[1], yerr=mean_energy_per_sim_last_std[1], fmt='o')
ax5.set_xlabel("Effective temperature")
ax5.set_ylabel("Energy")
ax6.scatter(T, mean_abs_mean_magnetisation[0])
ax6.errorbar(T, mean_abs_mean_magnetisation[0], yerr=mean_abs_mean_magnetisation_std[0], fmt='o')
ax6.scatter(T, mean_abs_mean_magnetisation[1])
ax6.errorbar(T, mean_abs_mean_magnetisation[1], yerr=mean_abs_mean_magnetisation_std[1], fmt='o')
ax6.set_xlabel("Effective temperature")
ax6.set_ylabel("Mean absolute magnetisation")
ax7.scatter(T, heat_capacitance[0])
ax7.errorbar(T, heat_capacitance[0], yerr=heat_capacitance_std[0], fmt='o')
ax7.scatter(T, heat_capacitance[1])
ax7.errorbar(T, heat_capacitance[1], yerr=heat_capacitance_std[1], fmt='o')
